<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0F2027,50:203A43,100:2C5364&height=200&section=header&text=SAGAR%20SHAKYA&fontSize=55&fontColor=ffffff&animation=fadeIn&fontAlignY=35" width="100%"/>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=24&duration=3000&pause=1000&color=36BCF7&center=true&vCenter=true&width=800&lines=Full-Stack+Java+Developer;MCA+Student;Spring+Boot+%7C+React+Developer;AI%2FML+%7C+Generative+AI+Enthusiast;Building+Real-World+Applications" />

</div>

---

# 👋 Hi, I'm Sagar Shakya

### 💻 Full-Stack Java Developer | 🤖 AI/ML Enthusiast | 🎓 MCA Student

I am an MCA student and Full-Stack Java Developer passionate about building real-world applications using **Java, Spring Boot, React, REST APIs, databases and AI/ML technologies**.

I enjoy learning new technologies, solving programming problems and turning ideas into practical software solutions.

---

<div align="center">

### 🚀 `Code • Learn • Build • Improve`

</div>

---

# 👨‍💻 About Me

```text
const sagar = {
    name        : "Sagar Shakya",
    education   : "MCA",
    role        : "Full-Stack Java Developer",
    backend     : ["Java", "Spring Boot", "Hibernate", "REST API"],
    frontend    : ["HTML", "CSS", "JavaScript", "React", "React Native"],
    databases   : ["MySQL", "MongoDB"],
    ai_ml       : ["Python", "Machine Learning", "NLP", "Generative AI"],
    cloud       : ["AWS", "Google Cloud", "Firebase"],
    tools       : ["Git", "GitHub", "Postman", "VS Code", "Eclipse"],
    goal        : "Build scalable and intelligent applications"
};
```

---

# 🧠 AI / ML & Generative AI

<div align="center">

<img src="https://skillicons.dev/icons?i=python,tensorflow,pytorch&perline=6" />

</div>

### 🔬 Areas I'm Exploring

`Machine Learning` • `Natural Language Processing` • `Generative AI` • `Deep Learning` • `Python` • `Data Analysis` • `AI Applications`

---

# ☕ Backend Development

<div align="center">

<img src="https://skillicons.dev/icons?i=java,spring,hibernate,maven&perline=8" />

</div>

**Core:** Java • OOP • Collections • Exception Handling • Multithreading

**Backend:** Spring Boot • REST API • Hibernate • JPA • MVC

---

# 🌐 Frontend Development

<div align="center">

<img src="https://skillicons.dev/icons?i=html,css,js,react,reactnative,bootstrap&perline=8" />

</div>

---

# 🗄️ Databases

<div align="center">

<img src="https://skillicons.dev/icons?i=mysql,mongodb,postgresql&perline=8" />

</div>

---

# ☁️ Cloud & Services

<div align="center">

<img src="https://skillicons.dev/icons?i=aws,gcp,firebase&perline=8" />

</div>

---

# 🛠️ Tools & Development Environment

<div align="center">

<img src="https://skillicons.dev/icons?i=git,github,vscode,eclipse,postman,maven&perline=8" />

</div>

---

# 🚀 Featured Projects

### 💇 Book My Salon

**React Native • Spring Boot • MySQL • Cloudinary**

A full-stack salon booking application with:

* 📅 Appointment booking
* 🏪 Merchant dashboard
* 🔔 Notification support
* 📊 Schedule management
* 🌐 REST APIs
* 🖼️ Cloudinary image management

---

### 🐾 Pet Adoption Portal

**Java • Spring Boot • MySQL • REST API**

A pet adoption management system for managing pets, users and adoption-related operations through REST APIs.

---

### 📈 StockSphere

**React • Spring Boot • MySQL**

Inventory and stock management application for managing products and stock efficiently.

---

### 💬 Chat Application

**Java • Socket Programming • Swing**

Desktop-based real-time chat application developed using Java socket programming and Swing UI.

---

### 📱 Phonebook Management System

**JSP • JDBC • MySQL**

CRUD-based phonebook management application using Java web technologies and MySQL.

---

# 🏆 Certifications & Achievements

| 🏅 Achievement                                                           | 📅 Year |
| ------------------------------------------------------------------------ | ------: |
| ☁️ Google Cloud Career Launchpad — Computing & Cybersecurity Foundations |    2026 |
| ☁️ AWS Academy Graduate — Cloud Foundations                              |    2026 |


---

# 📊 GitHub Analytics

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=sagarshakya27&show_icons=true&theme=tokyonight&hide_border=true&rank_icon=github" height="180"/>

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=sagarshakya27&layout=compact&theme=tokyonight&hide_border=true" height="180"/>

</div>

---



# 🏆 GitHub Trophies

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=sagarshakya27&theme=tokyonight&no-frame=true&no-bg=true&margin-w=6" />

</div>

---

# 🐍 Contribution Snake

<div align="center">

<img src="https://raw.githubusercontent.com/platane/snk/output/github-contribution-grid-snake.svg" />

</div>

---

# 🌱 Currently Learning

<div align="center">

`Spring Boot` • `Advanced Java` • `React` • `AI/ML` • `Generative AI` • `NLP` • `Cloud Computing` • `Data Structures & Algorithms`

</div>

---

# 🤝 Connect With Me

<div align="center">

<a href="https://github.com/sagarshakya27">
<img src="https://img.shields.io/badge/GITHUB-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

<a href="https://www.linkedin.com/in/sagar-shakya1/">
<img src="https://img.shields.io/badge/LINKEDIN-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="mailto:sagarshakya0838@gmail.com">
<img src="https://img.shields.io/badge/GMAIL-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

</div>

---

<div align="center">

### 💙 Thanks for visiting my profile!

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0F2027,50:203A43,100:2C5364&height=120&section=footer" width="100%"/>

</div>
